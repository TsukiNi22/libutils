...
class Middlewares<T, void>: private utils::security::observer::Observer<"Middlewares"> {
    public:
        ...
        /* adder */
        void addBefore(utils::pool::Middleware<T>& toAdd)                       {std::unique_lock lock(this->_lock); this->before.push_back(toAdd);}
        void addBefore(const std::vector<utils::pool::Middleware<T>>& toAdds)   {std::unique_lock lock(this->_lock); this->before.insert(before.end(), toAdds.begin(), toAdds.end());}
        void addAfter(utils::pool::Middleware<void>& toAdd)                     {std::unique_lock lock(this->_lock); this->after.push_back(toAdd);}
        void addAfter(const std::vector<utils::pool::Middleware<void>>& toAdds) {std::unique_lock lock(this->_lock); this->after.insert(after.end(), toAdds.begin(), toAdds.end());}

        /* caller */
        void callBefore(T arg) const
        {
            std::shared_lock lock(this->_lock);
            for (const utils::pool::Middleware<T>& middleware: this->before) {
                try {middleware(arg);}
        ...
        {
            std::shared_lock lock(this->_lock);
            for (const utils::pool::Middleware<void>& middleware: this->after) {
                try {middleware();}
                catch (const std::exception& e) {throw utils::exception::ErrorException(utils::exception::InternalCode::MiddlewareCall, e.what());}
            }
        }
