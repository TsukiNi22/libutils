...
inline utils::type::OVector2<T> operator+(const T& lhs, const utils::type::OVector2<U>& rhs)
...
// -------- Comparison (reverse) -------- //
template<typename T, typename U>
inline bool operator==(const T& lhs, const utils::type::OVector2<U>& rhs)
{return (lhs == rhs.x && lhs == rhs.y);}
...
// -------- Comparison (reverse) -------- //
template<typename T>
std::ostream& operator<<(std::ostream& os, const utils::type::OVector2<T>& v)
{return os << "(" << v.x << ", " << v.y << ")";}
