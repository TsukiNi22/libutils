...
template<typename T>
class Vector2: public utils::math::vector::IVector<T> {
    public:
        T x;
        T y;
...
auto operator+(const T& lhs, const utils::math::vector::Vector2<U>& rhs)
requires utils::concepts::AddableWith<T, U>
{
    using R = std::common_type_t<T, U>;
    return utils::math::vector::Vector2<R>(lhs + rhs.x, lhs + rhs.y);
}
...
// -------- Comparison (reverse) -------- //
template<typename T, typename U>
bool operator==(const T& lhs, const utils::math::vector::Vector2<U>& rhs)
...
// -------- Comparison (reverse) -------- //
template<typename T>
std::ostream& operator<<(std::ostream& os, const utils::math::vector::Vector2<T>& v)
{return os << "(" << v.x << ", " << v.y << ")";}
